...
-- Run this in Supabase SQL Editor before enabling the Vercel API routes.
...
create unique index if not exists seo_query_stats_unique_period_query
  on public.seo_query_stats (source, period_start, period_end, device_type, query, url_norm);
...
-- Admin UI reads these tables through Vercel API routes with the service role key.
